...
# define bug url
BUGURL="https://github.com/mickael-guene/asset_manifest/issues"
...
if [ "$WITHISL" ] ; then
    ISL_CONFIGURE="--with-isl=${TOP}/install_host"
else
    ISL_CONFIGURE="--without-isl"
fi
...
#isl
cd ${TOP}/build/isl
CFLAGS=$CFLAGS_TOOLSET ${TOP}/scratch/isl/configure         --prefix=${TOP}/install_host \
                                                            --with-gmp=${TOP}/install_host \
                                                            --disable-shared
make all -j${JOBNB}
make install
...
if [ "$STRIP" ] ; then
    ...
    tar -C $WDIR --atime-preserve -czf out/asset-toolset-${VERSION}-${ARCH}.tgz .
else
    tar -C install --atime-preserve -czf out/asset-toolset-${VERSION}-${ARCH}.tgz .
fi
